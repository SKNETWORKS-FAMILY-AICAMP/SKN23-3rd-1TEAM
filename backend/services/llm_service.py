...
class LLMService:
    def __init__(self):
        ...
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 {job_role} 직무의 10년 차 최고참 실무 면접관입니다.
    ...
    # ==========================================
    async def generate_tail_question(self, session_id: str, job_role: str, current_question: str, user_answer: str, turn: int):
        """
        지원자의 답변을 바탕으로 맞춤형 꼬리 질문을 비동기(Async)로 생성합니다.
        """
        # 1. 동적 RAG: 방금 대답을 DB에 기억시킴
        rag_service.append_interview_log(session_id, current_question, user_answer, turn)
