...
# 🚨 2. 문지기 출동! (무조건 헤더 그리기 전에 로그인 검증 및 복구)
...
# 🔥 중복 함수 제거 & 정상 작동하는 답변 처리 로직 하나만 남김!
...
st.markdown(
    ...
    * { font-family: 'Pretendard', sans-serif !important; box-sizing: border-box; }
    ...
    .ai-bubble {
        background: #ffffff; border: none; border-radius: 16px; padding: 16px 20px; color: #000; font-size: 15px; line-height: 1.6;
        box-shadow: 0 4px 24px rgba(0,0,0,0.07); max-width: 85%; align-self: flex-start; margin-bottom: 12px; white-space: pre-wrap; font-weight: 500;
    }
    .user-bubble {
        background: #bb38d0; border-radius: 16px; padding: 16px 20px; color: #ffffff; font-size: 15px; line-height: 1.6;
        box-shadow: 0 4px 24px rgba(0,0,0,0.07); max-width: 85%; align-self: flex-end; margin-bottom: 12px; white-space: pre-wrap; font-weight: 500;
    }
    ...
    [data-testid="stVerticalBlockBorderWrapper"] {
        background-color: #ffffff !important; border: none !important; border-radius: 16px !important;
        box-shadow: 0 4px 24px rgba(0, 0, 0, 0.07) !important; padding: 24px !important; transition: transform 0.2s ease, box-shadow 0.2s ease;
    }
    button[kind="primary"] { background: #bb38d0 !important; border: none !important; color: white !important; font-weight: 700 !important; border-radius: 16px !important; height: 50px !important; }
    button[kind="primary"]:hover { filter: brightness(1.1); box-shadow: 0 4px 24px rgba(0,0,0,0.07) !important;}
    [data-testid="stButton"] > button:not([kind="primary"]) { background: #fff !important; color: #555 !important; border: 1px solid #ddd !important; border-radius: 10px !important; height: 50px !important; font-size: 16px !important; font-weight: 600 !important; }
...
defaults = {
    ...
    "db_questions": [],
    "current_q_idx": 0,
    "user_id": user_id,  # 🔥 위에서 구한 정상적인 ID 할당
...
if not st.session_state.chatbot_started:
    ...
    persona_style = st.radio("면접관 스타일", ["깐깐한 기술팀장", "부드러운 인사담당자", "스타트업 CTO"])
    ...
    with col_start:
        if st.button("면접 시작하기", type="primary", use_container_width=True, disabled=disable_start):
            ...
            try:
                ...
                if len(db_questions) == 1: raise ValueError("질문 없음")
            except Exception:
...
if st.session_state.interview_mode == "text":
    ...
    # (Realtime HTML/JS 코드는 지우님이 작성하신 그대로 유지됩니다!)
    html = """
